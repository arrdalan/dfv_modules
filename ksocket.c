 ...
 *
 * @2014, Rice University, Houston, TX, USA
...
ksocket_t ksocket(int domain, int type, int protocol)
{
	...
	int ret = 0;
	//Ardalan
	int err, level, optname, flag, optlen;
	char *optval;
	...
	//Ardalan start
	/* disabling the Nagle's algorithm to reduce latency */
	level = 6; /* IPPROTO_TCP in glibc */
	optname = 1; /* TCP_NODELAY in glibc */
	flag = 1; /* disable */
	optval = (char *) &flag;
	optlen = sizeof(optval); 

	if (sk != NULL) {
/* not-for-release-start */
        	//err = security_socket_setsockopt(sk, level, optname);
                //if (err) {
                        ////goto out_puti;
		//}
		//else {
                	...
                	//if (level == SOL_SOCKET) {
                        	//err = sock_setsockopt(sk, level, optname, optval, optlen);
				//PRINTK1("dvserver_dvclient: used socket_setsockopt\n", err);
			//}
                	//else {
/* not-for-release-end */
		err = kernel_setsockopt(sk, level, optname, optval, optlen);
/* not-for-release-start */
				//PRINTK1("dvserver_dvclient: used sk->ops->setsockopt\n", err);
			//}
		//}
/* not-for-release-end */
        }
/* not-for-release-start */
	//PRINTK1("dvserver_dvclient: err return for sock_setsockopt = %d\n", err);
/* not-for-release-end */
	//Ardalan end
...
/*
 * receives bytes on "socket" until recv returns 0 (meaning the other end closed the socket).
 * @buffer: the data that will be received. Should be allocated to size of length before calling this function.
 * @length: max number of bytes to receive.
 * @flags: standard socket flags
 * @author: Kevin Boos
 */
ssize_t krecvall(ksocket_t socket, void *buffer, size_t length, int flags)
{
	int rcvd, total;

	if (!buffer)
	{
		buffer = kmalloc(length * sizeof(char), GFP_KERNEL);
		if (!buffer)
		{
			printk("ksocket: krecvall: error: couldn't kmalloc %d bytes for buffer\n", length);
			return -ENOMEM;
		}
	}
	...
	rcvd = 1;
	total = 0;
	while ((rcvd > 0) && (total < length))
	{
		rcvd = krecv(socket, (buffer + total), (length - total), MSG_WAITALL | flags);
		total += rcvd;
	}
